"""Where the potentials in problem_2.py came from.
...
#: and are checked anyway: they are the reason the answer is a round number.
CLAIMS = (
    ("I, the current through R (2 ohm), is 1 A", "r", "1"),
    ("the 10 ohm across the bottom carries nothing", "r_bottom", "0"),
    ("the 8 ohm spoke carries nothing", "r_top_right_spoke", "0"),
)


def verdict(held, solved) -> str:
    return f"ANSWER: I = {solved['r'].normalize()} A"
...
def main() -> int:
    result = elaborate(load_system(ROOT / "problem_2.py"), project_id="PRJ-EXAMPLES")
    if not result.ok:
    ...
    deck = lower_to_spice(
        result.snapshot, plan, traits=result.traits, title="problem_2"
    )
