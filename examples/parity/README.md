...
[`examples/blinky/`](../blinky/). Both toolchains get the same design, and
[`tests/test_parity.py`](../../tests/test_parity.py) asserts that what they produce agrees.

## What is compared, and what is not

A Fang build emits a KiCad netlist. An atopile build emits a `.kicad_pcb`, because atopile has
no netlist target and the layout is its artifact. Reduced to what a board *is*, both say the same
...
Fang gives it a net of its own. Neither is connectivity.
...
excuse for a difference. Both toolchains get the same parts, so a disagreement between them is a
