...
`ground`) and three parts, then joins them in `architecture`. The `>>` operator
is the only way parts get connected, and every connection is an entity in the
graph rather than a line in a file:
...
1 kOhm. They do not restate the values written above them. That is the
distinction the whole language rests on. A value is a choice, and a constraint
is the reason the choice is allowed.
