...
Given candidates, lowering picks one and produces the same answer every time,
in this process and the next. Determinism here is not a convenience: a pinout
that shifted between runs would make every downstream artifact unstable.
...
thresholds, drive against demand, pull-ups where the protocol needs them.
...
that is a different circuit, not the same one twice.
