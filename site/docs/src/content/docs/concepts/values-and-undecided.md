...
| `unknown` | Not known — and this is a value, not a gap |
...
expression cannot be stored, let alone evaluated — which is why nothing
downstream re-checks units.
...
"unknown poisons everything". `false and undecided` is `false`, because no value
of the unknown operand could make the conjunction true. `true or undecided` is
`true`. The kernel decides what it can and stops exactly where it must.
...
"there was nothing to tell" are different engineering situations, and collapsing
them is how a checklist comes to report full coverage it does not have.
