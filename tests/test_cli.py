...
COPPER = str(EXAMPLES / "copper_rules" / "copper_rules.py")
...
# -- 4.4 exporting the rules ----------------------------------------------


def test_export_rules_writes_a_design_rule_file(tmp_path):
    target = tmp_path / "copper_rules.kicad_dru"
    code = main(["export", COPPER, "--rules", "--project", "PRJ-CLI", "-o", str(target)])
    assert code == EXIT_OK

    text = target.read_text(encoding="utf-8")
    assert text.startswith("# Generated by fang ")
    assert "(version 1)" in text
    assert "(constraint track_width" in text


def test_export_rules_names_the_constraint_each_rule_projects(capsys):
    assert main(["export", COPPER, "--rules", "--project", "PRJ-CLI"]) == EXIT_OK
    output = capsys.readouterr().out
    assert output.count("(rule ") == 2
    assert '(rule "RULE-' in output


def test_export_net_classes_writes_the_assignments(capsys):
    assert main(["export", COPPER, "--net-classes", "--project", "PRJ-CLI"]) == EXIT_OK
    document = json.loads(capsys.readouterr().out)
    assert len(document["net_classes"]) == 1
    assert document["net_classes"][0]["projects"]


def test_export_rules_exits_non_zero_when_there_is_nothing_to_project(capsys):
    assert main(["export", DIVIDER, "--rules", "--project", "PRJ-CLI"]) == EXIT_FAILED
    assert "TOPO-0004" in capsys.readouterr().err


def test_export_without_a_flag_still_writes_the_netlist(capsys):
    assert main(["export", COPPER, "--project", "PRJ-CLI"]) == EXIT_OK
    assert capsys.readouterr().out.startswith('(export "version" "E"')


def test_rules_and_net_classes_are_mutually_exclusive():
    with pytest.raises(SystemExit):
        main(["export", COPPER, "--rules", "--net-classes", "--project", "PRJ-CLI"])
